    ...
    >
      <div ref={innerRef} className="flex whitespace-nowrap">
        {repeatedItems.map((item, i) => (
          <span
            key={i}
            className="flex items-center gap-6 px-6 text-2xl sm:text-3xl lg:text-4xl font-display font-bold text-[rgb(var(--ink) / 0.08)] select-none flex-shrink-0"
          >
            <span>{item}</span>
            <span className="w-1.5 h-1.5 rounded-full bg-[rgb(var(--accent) / 0.25)] flex-shrink-0" />
          </span>
        ))}
      </div>
    </div>
